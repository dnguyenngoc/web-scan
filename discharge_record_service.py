...
if __name__ == "__main__":
    SERVER_TYPE = 'discharge_record'
    ftp = FTP(username = config.FTP_USERNAME, password = config.FTP_PASSWORD, url = config.FTP_URL)
    while True:
        try:
            process = process_logic.read_by_status_name_and_type_join_load(db_session, 'upload', SERVER_TYPE)
            for item in process:
                ...
                for i in range(len(list_class)):
                    ...
                    ftp.create_file('/{server_type}/{document_id}/{field_name}.png'.format(server_type = SERVER_TYPE, document_id = document_id)
            process_crud.update(
                db_session, 
                document_id, 
